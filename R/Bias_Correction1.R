...
#   Rmax <- max(R)
#   dx <- 2*Rmax/ngrid
#   mygrid <- seq(-Rmax, Rmax, dx)
#   x.minus.mu <- sapply(seq_len(n), function(i) mygrid-mu[i])
#   Fchi2.grid <- stats::pchisq(mygrid, K)
#   phi.grid <- stats::dnorm(x.minus.mu)
#   h.grid <- sweep(phi.grid, 1, Fchi2.grid, "*")
#   S.top <- colSums(sweep(h.grid, 1, mygrid, "*"))
#   S.bottom <-
# }
# K <- 3; R <- 2:4; mu <- round( stats::runif(3), 2)

Sfunc1 <- function(mu, R, K, ngrid = 200) {
  dx <- 2 * R / ngrid
  mygrid <- seq(-R, R, dx)
  phi.grid <- stats::dnorm(mygrid - mu)
  Fchi2.grid <- stats::pchisq(R^2 - mygrid^2, K - 1)
...
#   mus <- sapply(
#     X = mus.trim,
#     FUN = function(x) Newton1(mu.trim, RR, K, gammahat, ngrid = ngrid, tol = tol, max.iter = max.iter)
#   )
#   return(mus)
# }
