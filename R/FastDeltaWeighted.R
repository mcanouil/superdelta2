...
superdelta2 <- function(mydata, offset = 1.0, Grps, W = NULL, trim = .2, adjp.thresh = 0.05, prop = 1.0) {
  ...
  # for (g in seq_len(G)) {
  #   GLBNor.WW[,g] <- sum(W[(Nc[g]+1):Nc[g+1]]) *
  #     apply(
  #       X = GLBNor.res.sq[,(Nc[g]+1):Nc[g+1]],
  #       MARGIN = 1,
  #       FUN = w_mean,
  #       w = W[(Nc[g]+1):Nc[g+1]]
  #     )
  ...
  ## print(G)
  Rlist <- lapply(seq_len(G), function(g) sqrt(W.g[g]) * outer(Zbar.g[, g], Zbar.g[, g], "-"))
  ...
  MeanDiffs <- sapply(seq_len(ncol(pairs)), function(j) {
    a <- pairs[1, j]
    b <- pairs[2, j]
  ...
  t.const <- sapply(seq_len(ncol(pairs)), function(j) {
    a <- pairs[1, j]
    b <- pairs[2, j]
  ...
  if (is.null(rownames(mydata))) {
    rownames(mydata) <- seq_len(m)
  }
  names(Fstats) <- rownames(tstats) <- rownames(mydata)
  ## p-values and multiple testing adjustment (based on BH)
  pvalues.F <- stats::pf(Fstats, G - 1, N - G, lower.tail = FALSE)
  padj.F <- stats::p.adjust(pvalues.F, method = "BH")
  ## two-sided t-test is F-test with df1 = 1
  pvalues.t <- stats::pf(tstats^2, 1, N - G, lower.tail = FALSE)
  ...
  # }
  padj.t <- matrix(stats::p.adjust(pvalues.t, method = "BH"), nrow = nrow(tstats))
  rownames(padj.t) <- rownames(tstats)
  colnames(padj.t) <- colnames(tstats)
  ## Output of Top and Bottom may not be necessary later on.
  return(list(
    SigGenes = SigGenes, Fstats = Fstats, pvalues.F = pvalues.F, padj.F = padj.F,
