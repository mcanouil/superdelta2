...
FastFChi2 <- function(x, df, R2max, ngrid = 200) {
  m <- nrow(x)
  dx <- R2max / ngrid
  chi2grid <- seq(0, R2max, dx)
  chi2vec <- stats::pchisq(chi2grid, df)
  ## use rule=2 for cases in which x is slightly larger than R2max due
  ## to numerical errors.
  return(matrix(stats::approx(chi2grid, chi2vec, xout = x, rule = 2)[["y"]], m))
}
...
Newton <- function(mu.trim, R, K, ngrid = 200, tol = 1e-3, niter = 3) {
  ...
  mu.now <- mu.trim
  dx <- 2 * R / ngrid
  mygrid <- t(sapply(seq_len(m), function(i) seq(-R[i], R[i], dx[i])))
  ...
  xFchi2 <- mygrid * Fchi2
  x2Fchi2 <- mygrid * xFchi2
  Bmu <- 1
  for (k in seq_len(niter)) {
    phi <- stats::dnorm(sweep(mygrid, 1, mu.now))
    ## dx gets cancelled; so no need to multiply each term by dx
    Bmu <- rowSums(Fchi2 * phi)
    Tmu <- rowSums(xFchi2 * phi)
    Hmu <- rowSums(x2Fchi2 * phi)
    Smu <- Tmu / Bmu
...
SphereTrim <- function(Rlist, CenterMat, trim, sigma, prop = 1.0, bias.correct = FALSE, ...) {
  ...
  G <- length(Rlist)
  ## re-center
  Rlist2 <- lapply(seq_len(G), function(g) sweep(Rlist[[g]], 1, CenterMat[, g]))
  if (prop < 1) {
    idx1 <- sample(m, round(m * prop))
    trimmed.m <- round(length(idx1) * (1 - trim), 0)
    Lx2 <- Reduce("+", Map(function(x) x[, idx1]^2, Rlist2))
    threshs <- matrixStats::rowOrderStats(Lx2, which = trimmed.m + 1)
  } else {
    trimmed.m <- round((m - 1) * (1 - trim), 0)
    Lx2 <- Reduce("+", Map(function(x) x^2, Rlist2))
    threshs <- matrixStats::rowOrderStats(Lx2, which = trimmed.m + 2)
    idx1 <- seq_len(m)
  }
  IDX <- sweep(Lx2, 1, threshs, "<")
  Rtrim1 <- do.call(cbind, Map(function(x) rowSums(x[, idx1] * IDX), Rlist2)) / trimmed.m
  if (bias.correct) {
    ## bias correction
    mu2 <- sqrt(rowSums(Rtrim1^2)) / sigma
    R.normalized <- sweep(Rtrim1, 1, mu2, "/")
    RR <- sqrt(threshs) / sigma
    ## BiasCorrection.results <- t(sapply(seq_len(m), function(i) Newton1(mu2[i], RR[i], K=G-1, ...)))
